...
def process_pokemon(pokemon_ref, session, config):
    """Fetches, processes, and structures data for a single Pokémon."""
    try:
        response = session.get(pokemon_ref["url"], timeout=config["timeout"])
        response.raise_for_status()
        pokemon_data = response.json()

        species_url = pokemon_data["species"]["url"]
        species_response = session.get(species_url, timeout=config["timeout"])
        ...
        output_path = config["output_dir_pokemon"]
        ...
        return f"Request failed for {pokemon_ref['name']}: {e}"
...
def main(config, session):
    ...
    parser_config = {
        "item_name": "Pokemon",
        "master_list_url": f"{config['api_base_url']}pokemon?limit=1500",
        "processing_func": process_pokemon,
        "session": session,
        "config": config,
    }
